...
namespace Trementa.OpenApi.Generator.Generators.CodeGenerators
{
    public abstract class StaticCodeFileGenerator : CodeGenerator
    {
        ...
        protected IEnumerable<string> GetAllFiles()
        {
            ...
            if (Options.TemplateFolder != null && Directory.Exists(Options.TemplateFolder))
                foreach (var directory in Directory.EnumerateDirectories(Options.TemplateFolder, "*",
                    SearchOption.TopDirectoryOnly))
                    foreach (var file in Directory.EnumerateFiles(directory, "*", SearchOption.AllDirectories))
                    {
                        var templatePath = Path.GetRelativePath(Options.TemplateFolder, file);
        ...
        protected Stream GetGeneralStream(string templatePath)
        {
            ...
            if (Options.TemplateFolder == null)
                throw new FileNotFoundException($"Couldn't find \"{templatePath}\"");
            return new FileStream(Path.Combine(Options.TemplateFolder, templatePath), FileMode.Open, FileAccess.Read, FileShare.Read, 4096,
                FileOptions.Asynchronous | FileOptions.SequentialScan);
        }
