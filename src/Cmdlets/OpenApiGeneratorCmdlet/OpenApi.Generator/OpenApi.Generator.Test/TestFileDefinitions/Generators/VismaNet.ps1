...
try
{
    ...
    $swagger = Invoke-WebRequest -Uri "https://integration.visma.net/API-index/doc/swagger" -Headers @{"Accept"="application/json,*/*"; "Accept-Encoding"="gzip, deflate, br"} -ContentType "application/json" -UseBasicParsing

    $swaggerJson20 = $swagger.Content -replace '(.*?)documentNumber(.*)', '$1_documentNumber$2' -replace "projectId", "_projectId1"
    $swaggerJson = $swaggerJson20 | ConvertFrom-Json

    if($swaggerJson.openapi -lt "3.0")
    {
        "Saving received specification..."
        $swaggerJson20 |Format-Json | Out-File ../swaggerVismaNet.json -Encoding ASCII -Force
...
"Save documentation..."
$swaggerJson30 |Format-Json | Out-File ../openApiVismaNet.json -Encoding ASCII -Force
