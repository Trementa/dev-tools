...
try
{
    ...
    #$swagger = Invoke-WebRequest -Uri "https://137.135.134.2/swagger/v1/swagger.json" -Headers @{"Accept"="application/json,*/*"; "Accept-Encoding"="gzip, deflate, br"} -ContentType "application/json" -UseBasicParsing
    $swagger = (Get-Content ..\swaggerSpeedyCraftApi2.0.json) 
    $swaggerJson20 = $swagger -replace "{assignmentIdExt}", "{assignmentIdExt2}"
    $swaggerJson = $swaggerJson20 | ConvertFrom-Json
    if($swaggerJson.openapi -lt "3")
    {
        "Saving received specification"
        #$swaggerJson20 |Format-Json | Out-File ../swaggerSpeedyCraftApi2.0.json -Encoding ASCII -Force
...
"Save documentation..."
$swaggerJson30 |Format-Json | Out-File ../openApiSpeedyCraftApi30.json -Encoding ASCII -Force
